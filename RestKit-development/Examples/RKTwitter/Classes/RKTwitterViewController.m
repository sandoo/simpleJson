...
- (void)loadTimeline
{
    webserviceStructure *service = [webserviceStructure sharedService];
    NSString *pattern = [service GetIndexWithSuccess:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
        
    } failure:^(RKObjectRequestOperation *operation, NSError *error) {
        
    }];
//    [[webserviceStructure sharedService] cancelAllRequests];
    return;
    // Load the object model via RestKit
    RKObjectManager *objectManager = [RKObjectManager sharedManager];

    [RKMIMETypeSerialization registerClass:[RKURLEncodedSerialization class] forMIMEType:RKMIMETypeFormURLEncoded];
